(() => {
  ...
  function fetchProjects() {
    ...
    if (!projectsPromise) {
      projectsPromise = fetch(projectsEndpoint, { credentials: 'same-origin' })
        .then((response) => {
          if (!response.ok) {
            throw new Error(`Failed to load ${projectsEndpoint}`);
          }
  ...
  function hydrateProjectDetail(projects) {
    ...
    const ctas = {
      ...
      contactEmail: detailRoot.dataset.contactEmail || 'hello@mohamedalimabrouki.com'
    };
